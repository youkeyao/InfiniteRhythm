...
public static class SceneChangeGenerator
{
    const int SubBandsNum = 4;
    const int HistorySize = 10;

    public static float minInterval = 15f;
    public static int windowSize = 65536;
    public static int moveLength = 131072;
    ...
    static Queue<float> s_changeTimes = new Queue<float>();
    ...
    public static Queue<float> GetChangeTimes() => s_changeTimes;

    public static void Generate(float[] samples, int sampleRate, float timeOffset)
    {
        Debug.Log((samples.Length - windowSize * 2) / moveLength);
            ...
            for (int sampleIndex = 0; sampleIndex < samples.Length; sampleIndex += moveLength)
            {
                ...
                for (int i = 0; i < SubBandsNum; i++)
                {
                    if (currentTime > restTime)
                    {
                        ...
                        {
                            Debug.Log(subBandEnergies[i] / (C * average));
                            s_changeTimes.Enqueue(currentTime);
